...
LLM agents consume text, but brain signals are tensors and screen pixels are images. Using **YOLO + OCR** to convert the screen into structured text, and training a **brain signal encoder** to convert EEG into a discrete label, we integrate **text-only LLM** that reasons over both streams to reduce cumulative costs.
